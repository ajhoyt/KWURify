    ...
    def get_artist_popularities(self, artist_ids):
        ...
        while True:
            ...
            response = requests.get(
                f"https://api.spotify.com/v1/artists/?ids={ids_str}&limit=50&offset={iteration*50}",
                headers=headers,
            )
...
def get_token():
    ...
    print(
        f"Token expires at: {token_info['expires_at']}, Current time: {now}, Is expired: {is_expired}"
    )
    if is_expired:
        # Refresh the token
        headers = {
            "Authorization": "Basic "
            + base64.b64encode(
                f'{os.getenv("CLIENT_ID")}:{os.getenv("CLIENT_SECRET")}'.encode("utf-8")
            ).decode("utf-8"),
            "Content-Type": "application/x-www-form-urlencoded",
        }
        response = requests.post(
            f'{API_URL}&grant_type=refresh_token&refresh_token={token_info["refresh_token"]}',
            headers=headers,
        )
...
def get_recommendations(df: pd.DataFrame):
    ...
    while sum(df_recs["recommendation_duration"]) < airtime_ms:
        ...
        if remaining_time < BATCH_LIMIT:
            ...
            batch_time_limit = BATCH_LIMIT
        print(
            "current recommendation duration: ", sum(df_recs["recommendation_duration"])
        )
        ...
        if batch_end_seed == (len(df["track_id"])):
            ...
            seed_track_ids = (
                df["track_id"].iloc[batch_start_seed:batch_end_seed].tolist()
            )
        ...
        print(
            "current batch time remaining before next batch: " + str(batch_time_limit)
        )
        response_json = response.json()
        print(url)
        print(response_json)
        ...
        for track in response_json["tracks"]:
            ...
            if (
                (sum(df_recs["recommendation_duration"])) + track_duration_ms
            ) > airtime_ms:
                #####end_while = True
                return df_recs
            ...
            new_row = {
                "recommendation_ids": track["id"],
                "recommendation_uris": track["uri"],
                "recommendation_names": track["name"],
                "recommendation_popularities": track["popularity"],
                "recommendation_duration": track_duration_ms,
            }
            df_recs.loc[len(df_recs)] = new_row

            if (
                True
                in df_recs.duplicated(
                    subset=[
                        "recommendation_ids",
                        "recommendation_names",
                        "recommendation_popularities",
                ...
                df = df.drop_duplicates(
                    subset=["recommendation_ids", "recommendation_names"],
                    ignore_index=True,
                )
                continue
            ...
            print(
                f'new recommendation duration after adding {track["name"]} to the setlist: ',
                sum(df_recs["recommendation_duration"]),
            )
...
def create_playlist(df_recs: pd.DataFrame):
    ...
    return render_template(
        "playlist_embed.html", playlist_iframes=playlist_iframes
    )  # body
...
# Load the base64 encoded secret key from the environment variable
app_secret_base64 = os.getenv("APP_SECRET_KEY")
if app_secret_base64:
    app.secret_key = base64.b64decode(app_secret_base64)
else:
    raise ValueError("APP_SECRET_KEY environment variable is not set or is invalid")
...
def redirect_page():
    ...
    response = requests.post(
        f'{API_URL}?client_id={os.getenv("CLIENT_ID")}&client_secret={os.getenv("CLIENT_SECRET")}&grant_type=authorization_code&code={code}&redirect_uri={REDIRECT_URI}',
        headers=headers,
    )
    session["TOKEN_INFO"] = response.json()
...
def choose_playlist():
    ...
    iteration = 0
    while True:
        curr_user_playlists_response = requests.get(
            f"https://api.spotify.com/v1/me/playlists?limit=50&offset={iteration*50}",
            headers=headers,
        )
        curr_user_playlists_json = curr_user_playlists_response.json()
...
def generate_setlist():
    ...
    df = pd.json_normalize(
        ...
        record_path="artists",
        meta_prefix="track_",
        meta=["name", "id", "popularity", "duration_ms"],
    )
    ...
    df = (
        ...
        .agg({"artist_name": lambda x: list(x), "artist_id": lambda x: list(x)})
        .rename({"artist_name": "artists_names"}, axis=1)
        .reset_index()
    )
    ...
    return create_playlist(
        df_recs=df_recs
    )  # render_template('index.html', dataframe=df_recs.to_html(),duration_sum = duration_sum)
